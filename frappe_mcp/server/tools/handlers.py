...
def handle_call_tool(params, tool_registry: OrderedDict[str, tools.Tool]):
    ...
    if tool_name not in tool_registry:
        ...
        error_content = types.TextContent(
            text=f"Tool '{tool_name}' not found.", _meta=None
        )
    ...
    if not fn:
        error_content = types.TextContent(
            text=f"Tool '{tool_name}' has no associated function.", _meta=None
        )
        result = types.CallToolResult(content=[error_content], isError=True)
        return result.model_dump(exclude_none=True, by_alias=True)
    ...
    try:
        ...
        if isinstance(tool_result, dict):
            content = types.TextContent(text=str(tool_result), _meta=None)
        ...
        error_content = types.TextContent(
            text=f"Error calling tool '{tool_name}': {e}", _meta=None
        )
        result = types.CallToolResult(content=[error_content], isError=True)
        return result.model_dump(exclude_none=True, by_alias=True)
...
def handle_list_tools(params, tool_registry: OrderedDict[str, tools.Tool]):
    ...
    tool_list = [types.Tool.model_validate(tool) for tool in tool_registry.values()]
